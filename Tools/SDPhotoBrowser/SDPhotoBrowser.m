...
- (void)setupToolbars
{
    ...
    // 2.closeBtn
    closBtn = [[UIButton alloc] init];
//    [closBtn setTitle:@"X" forState:UIControlStateNormal];
    [closBtn setImage:[UIImage imageNamed:@"关闭"] forState:UIControlStateNormal];
    [closBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    closBtn.backgroundColor = [UIColor colorWithRed:0.1f green:0.1f blue:0.1f alpha:0.90f];
    closBtn.layer.cornerRadius = 5;
    closBtn.clipsToBounds = YES;
    [closBtn addTarget:self action:@selector(dissmiss) forControlEvents:UIControlEventTouchUpInside];
    [self addSubview:closBtn];
    
    
    closBtn.frame = CGRectMake( 30 , self.bounds.size.height - 70, 50, 25);
    closBtn.center = CGPointMake(45, 35);
...
- (void)layoutSubviews
{
    ...
    _saveButton.frame = CGRectMake(SCREEN_WIDTH- 30 - 50, self.bounds.size.height - 70, 50, 25);
}
